...
class DictOptimizer:
    ...
    def __init__(self, filepath: str, delimiter: str = " ",
                 ...
                 filter_non_cjk: bool = True, force: bool = False):
    ...
    def _find_entry_start(self, lines: list) -> int:
        for i, line in enumerate(lines):
            if line.strip() == '...':
                return i + 1
        if not self.force:
            print("[错误] 未找到 '...' 词条起始标记，文件可能不是标准 Rime 词库。"
                  "使用 --force 可强制从文件开头处理（头部非注释行可能被误判为词条）",
                  file=sys.stderr)
            sys.exit(1)
        print("[警告] 未找到 '...' 词条起始标记，--force 已指定，"
              "将从文件开头处理（头部非注释行可能被误判为词条）", file=sys.stderr)
        return 0
...
示例:
    ...
    parser.add_argument('--force', action='store_true', default=False,
                        help='未找到 ... 起始标记时强制从文件开头处理（默认报错退出）')
